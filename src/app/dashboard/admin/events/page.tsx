        ...
        <div className="max-w-6xl space-y-10">
            ...
            <div className="flex justify-between items-end">
                ...
                <button
                    ...
                    className="bg-orange-600 text-white px-8 py-3 rounded-2xl font-black uppercase text-sm tracking-widest hover:bg-orange-700 shadow-xl shadow-orange-200 transition-all active:scale-95"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {events.map((event, i) => (
                    <div key={i} className={`bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-sm relative group hover:shadow-2xl transition-all duration-500 ${event.status === 'Active' ? 'ring-2 ring-orange-500/10' : ''}`}>

                        {/* Status Badge */}
                        <div className="absolute top-8 right-8">
                            <span className={`text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-[0.15em] shadow-sm ${event.status === 'Active' ? 'bg-orange-100 text-orange-700 border border-orange-200' :
                                    event.status === 'Planned' ? 'bg-orange-100 text-orange-700 border border-orange-200' :
                        ...
                        <div className="mb-10">
                            <div className="text-orange-600 font-black text-xs uppercase tracking-[0.2em] mb-2">{event.date}</div>
                            <h3 className="text-2xl font-black text-slate-900 group-hover:text-orange-600 transition-colors uppercase leading-tight">{event.title}</h3>
                        </div>
                        ...
                        <div className="mt-8 flex gap-3">
                            ...
                            <button className="flex-1 bg-orange-600 text-white font-bold text-xs py-3 rounded-xl hover:bg-slate-800 uppercase tracking-widest">Pricing Panel</button>
                        </div>
                    </div>
                ))}
            </div>

            <div className="bg-orange-600 rounded-[2.5rem] p-10 text-white relative overflow-hidden">
                <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div className="lg:col-span-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
                            <div className="p-6 bg-white/5 rounded-2xl border border-white/10 hover:border-white/20 transition-colors">
                                <div className="font-black uppercase text-orange-400 text-xs mb-1">Standard PAYG</div>
                            ...
                            <div className="p-6 bg-orange-600 rounded-2xl border border-orange-400 shadow-xl shadow-orange-900/40">
                                <div className="font-black uppercase text-orange-100 text-xs mb-1">Yearly PRO</div>
                                <div className="text-2xl font-black mb-2">5% COMMISSION</div>
                                <div className="text-[10px] text-orange-200 font-bold">£1,200 ANNUAL ACCESS</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-1 flex items-center justify-center">
                        <div className="w-48 h-48 rounded-full border-8 border-white/5 flex flex-col items-center justify-center text-center">
                            ...
                            <div className="text-[10px] text-orange-400 font-black">+£12 YOY</div>
                        </div>
                    </div>
                </div>
                {/* Decor */}
                ...
                <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); }}>
                    <div className="space-y-4">
                        <div>
                            ...
                            <input type="text" className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl font-bold outline-none focus:border-orange-600 transition-all" placeholder="e.g. Winter Tech Summit" />
                        </div>
                    ...
                    <div className="pt-4 flex gap-4">
                        ...
                        <button type="submit" className="flex-1 bg-orange-600 text-white py-3 rounded-xl font-black uppercase text-xs shadow-lg shadow-orange-200">Confirm Schedule</button>
                    </div>
                </form>
            </Modal>
